...
**AI编程交互规则**: 当完成代码提交并准备部署时，**AI必须主动评估并提示用户选择合适的部署方案**。

#### 4.5.1 三种部署方案对比

| 方案 | 名称 | 使用场景 | 推荐度 | 部署时间 | 前提条件 |
|------|------|---------|--------|---------|---------|
| **方案1** | 本地编译+上传 | 日常开发，SSH可用 | ⭐⭐⭐⭐⭐ | 2分钟 | 本地Node.js |
| **方案2** | Git提交编译产物 | SSH不可用，紧急情况 | ⭐⭐ | 1分钟 | Git可用 |
| **方案3** | 服务器端编译 | 服务器有Node.js环境 | ⭐⭐⭐⭐ | 3分钟 | 服务器Node.js |

**详细文档**: `docs/三种部署方案完整指南.md`

#### 4.5.2 AI部署决策规则

**规则1: 默认推荐方案1**

除非满足特殊条件，否则AI应默认推荐方案1：
...
如果用户提到SSH不可用或网络受限，AI应推荐方案2：

```
检测到SSH可能不可用，推荐使用方案2：

⚠️ 方案2 - Git提交编译产物（备用方案）

执行步骤：
  1. 切换配置：cp .gitignore .gitignore.backup && cp .gitignore.allow-dist .gitignore
  2. 编译并提交：npm run build:h5 && git add dist/ && git commit -m "chore: 添加编译产物"
  3. 服务器拉取：ssh root@111.48.74.245 "cd /tmp/AITY/aity-uni-app-v2 && git pull && cp -r dist/build/h5/* /var/www/html/h5/"

详细文档：docs/方案2-Git提交编译产物指南.md

注意：此方案会导致Git仓库体积增大，不建议作为常规方案。
```
...
如果服务器已安装Node.js（或用户正在安装），AI应同时提供方案1和方案3供选择：

```
检测到服务器有Node.js环境，提供两种方案：
...
根据当前的代码修改情况，我为您推荐以下部署方案：

【方案1: 本地编译+上传】（推荐⭐⭐⭐⭐⭐）
  适用场景：日常开发，SSH可用
  执行命令：cd scripts && full-deploy.bat
  部署时间：约2分钟
  优点：快速、可靠、本地可测试

【方案3: 服务器端编译】（备选⭐⭐⭐⭐）
  适用场景：服务器有Node.js环境
  执行命令：cd scripts && deploy-server-build.bat
  部署时间：约3分钟
  优点：不需要本地编译
