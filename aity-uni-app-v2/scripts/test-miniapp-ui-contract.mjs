...
const [filterBar, marketPage, marketLadderPage, webviewPage, aiAdvisorPage, agentConfigPage, profilePage, navBar] = await Promise.all([
	...
	read('src/pages/admin/agent-config.vue'),
...
const agentLocalState = await read('src/utils/ai-advisor-config.js')
...
assert.match(aiAdvisorPage, /<app-nav-bar title="Agent" show-back/, 'Agent 必须使用安全区导航')
assert.match(aiAdvisorPage, /getAgentConfigApi/, 'Agent 页面必须动态读取安全配置')
assert.match(aiAdvisorPage, /sendAgentMessageApi/, 'Agent 页面必须通过后端统一对话接口发送消息')
assert.doesNotMatch(aiAdvisorPage, /sendAIMessage\(/, 'Agent 页面不得继续使用旧图灵代理')
assert.match(aiAdvisorPage, /messages\.value\.pop\(\)/, 'Agent 请求失败后必须移除输入中占位消息')
assert.doesNotMatch(agentLocalState, /wenda|buildRequestBody|getThinkMode|tdx-auth/, 'Agent 本地状态不得保留旧问小达请求配置')
assert.match(agentConfigPage, /getAvailableAgentsApi/, 'Agent 管理必须读取已授权 Agent 列表')
assert.match(agentConfigPage, /getAgentUsageApi/, 'Agent 管理必须展示调用统计')
assert.doesNotMatch(agentConfigPage, /API Key|apiKey|密钥/, 'Agent 管理页面不得展示或编辑密钥')
...
assert.match(profilePage, /Agent 管理/, '管理员必须有 Agent 管理入口')
