...
-- 创建时间: 2026-04-15
-- 说明：
--   1. 使用新数据库名称 "投研图灵室_v2"，便于与旧数据库合并
--   2. 包含完整的表结构定义
--   3. 包含基础测试数据（用户权限、AI配置等）
--   4. 如果找到旧数据库，可以通过以下方式合并：
--      - INSERT INTO 投研图灵室_v2.users SELECT * FROM 投研图灵室.users;
-- =====================================================
...
SELECT '表数量: 10' AS '📁';
SELECT '下一步操作:' AS '📝';
SELECT '1. 更新 .env 文件中的 DB_NAME=投研图灵室_v2' AS '   ';
SELECT '2. 更新AI配置的API密钥' AS '   ';
SELECT '3. 启动后端服务: npm start' AS '   ';
SELECT '========================================' AS '';
