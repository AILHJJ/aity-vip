...
async function addDiscussionReply(req, res) {
  try {
    ...
    if (req.body.images && Array.isArray(req.body.images) && req.body.images.length > 0) {
      replyData.images = req.body.images;
    }
...
async function updateDiscussion(req, res) {
  try {
    ...
    const discussion = await Discussion.findByPk(id);
    if (!discussion) {
      return res.status(404).json(notFound('讨论不存在'));
    }

    // 权限检查：仅管理员或发帖人可编辑
    const isCreator = discussion.userId === currentUserId;
    if (!isAdmin && !isCreator) {
      return res.status(403).json(forbidden('无权编辑此讨论'));
    }
...
async function deleteDiscussion(req, res) {
  try {
    ...
    const isCreator = discussion.userId === currentUserId;
    if (!isAdmin && !isCreator) {
      return res.status(403).json(forbidden('无权删除此讨论'));
    }

    // 联合删除所有回复
    await DiscussionReply.destroy({ where: { discussionId: id } });
    await DiscussionFavorite.destroy({ where: { discussionId: id } });
    await discussion.destroy();

    res.json(success(null, '删除成功'));
  } catch (err) {
    console.error('[删除讨论] 错误:', err);
    res.status(500).json(error('服务器错误'));
  }
}

// 编辑回复
async function updateDiscussionReply(req, res) {
  try {
    ...
    if (!reply) {
      return res.status(404).json(notFound('回复不存在'));
    }

    // 验证回复属于该讨论
    if (reply.discussionId !== parseInt(id)) {
      return res.status(400).json(badRequest('回复不属于该讨论'));
    }

    // 权限检查：仅管理员或回复者可编辑
    const isCreator = reply.userId === currentUserId;
    if (!isAdmin && !isCreator) {
      return res.status(403).json(forbidden('无权编辑此回复'));
    }
...
async function deleteDiscussionReply(req, res) {
  try {
    ...
    const isCreator = reply.userId === currentUserId;
    if (!isAdmin && !isCreator) {
      return res.status(403).json(forbidden('无权删除此回复'));
    }

    await reply.destroy();

    // 更新讨论的回复数和状态
    const remainingCount = await DiscussionReply.count({ where: { discussionId: id } });
    if (remainingCount === 0) {
      await Discussion.update({ status: 'pending' }, { where: { id } });
    }

    res.json(success(null, '删除成功'));
  } catch (err) {
    console.error('[删除回复] 错误:', err);
    res.status(500).json(error('服务器错误'));
  }
}
